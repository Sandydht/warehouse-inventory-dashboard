...
class AddedProduct {
  ...
  private _verifyPayload(
    ...
    InputValidator.requireNotBlank(
      id,
      ADDED_PRODUCT_ERRORS.NOT_CONTAIN_NEEDED_PROPERTY,
    );
    InputValidator.requireNotBlank(
      sku,
      ADDED_PRODUCT_ERRORS.NOT_CONTAIN_NEEDED_PROPERTY,
    ...
    InputValidator.skuValidFormat(sku, ADDED_PRODUCT_ERRORS.SKU_INVALID_FORMAT);
    InputValidator.positiveNumberValidFormat(
      price,
      ADDED_PRODUCT_ERRORS.PRODUCT_PRICE_MUST_BE_POSITIVE_NUMBER,
    );
    InputValidator.positiveNumberValidFormat(
      quantity,
